...
export type CreateDebtStoreOptions = {
  /** Persistence key. Use "debtos-v1" for parity across web and mobile. */
  name: string;
  /** Platform storage adapter — createJSONStorage(() => localStorage | AsyncStorage). */
  storage: PersistStorage<DebtState> | undefined;
};

/**
 * Platform-agnostic DebtOS store. Web injects localStorage, mobile injects
 * AsyncStorage. All action logic is identical and lives here (single source of
 * truth shared by both apps).
 */
export function createDebtStore(opts: CreateDebtStoreOptions) {
  return create<DebtStore>()(
    persist(
      (set, get) => ({
